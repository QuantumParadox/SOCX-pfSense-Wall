...
- Adds `socx resilience` and `socx range-plan`: a read-only recovery-readiness drill and an approval-only isolated cyber-range VLAN plan. Neither command restores configuration, creates VLANs, or changes firewall policy.
...
- Adds Research-SOC pages through `/coverage`, `/hunts`, `/rules-lab`, `/soc-score`, and `/model-tournament` for ATT&CK visibility, guided hunt hypotheses, draft-only detection rules, NIST/CISA-style maturity scoring, and Pi AI model-route comparisons.
- Keeps the Detection Rule Lab approval-gated: Suricata, Sigma, YARA, pfBlockerNG, and device-profile outputs are drafts or review ideas only and are never installed automatically.

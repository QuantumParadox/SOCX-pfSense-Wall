...
function renderFlightRecorder(state, recorder) {
  ...
  root.innerHTML = [
    ...
    card("Recorder Confidence", [
      ...
      kv("Recorder Schedule", recorder.schedule?.state || "WAIT", recorder.schedule?.state === "OK" ? "green" : "yellow"),
      kv("Service Assurance", recorder.assurance?.state || "WAIT", recorder.assurance?.state === "OK" ? "green" : "yellow"),
      kv("SHA-256 Manifests", recorder.manifest_count || 0, Number(recorder.manifest_count || 0) ? "green" : "yellow"),
    ].join("")),
    card("Integrity Checks", table(["Check", "State", "Detail"], (recorder.integrity || []).map((row) => [row.check || "--", row.state || "--", row.detail || "--"]))),
    card("Coverage Map", table(["Day", "Retained Samples"], (recorder.coverage || []).map((row) => [row.day || "--", row.samples || 0]))),
